...
def login(user_id, user_name):
    #유저아이디로된 디렉토리 생성
    path = '.\\라이어전용폴더\\' + user_id
    os.makedirs(path)

    user_info = ".\\라이어전용폴더\\" + user_id + "\\" + user_id + "info.txt"
...
def createroom(NT2, nsnum, us):
    ...
    room = ".\\라이어서버전용폴더\\" + nsnum + '\\' + nsnum + ".txt"
    roomdate = open(room, 'w', encoding="UTF-8")
    roomdate.write(rname +"\n" + rcode + "\n" + rstart + "\n"+ rday +"\n" + rtime + "\n" + ruserc + "\n" + sr + "\n" + dieuser + "\n" + user + "\n")
    roomdate.close() 

    user_list = ".\\라이어서버전용폴더\\" + nsnum + "\\" + nsnum + "user.txt"
    roomdate = open(user_list, 'w', encoding="UTF-8")
    roomdate.close() 

    room_owner = ".\\라이어서버전용폴더\\"+ nsnum + "\\" + us + "owner.txt"
...
def createuserinfo(us, rcode):
    ...
    user_info = ".\\라이어전용폴더\\" + us + "\\" + us + "info.txt"
    room = ".\\라이어서버전용폴더\\" + rcode + '\\' + rcode + ".txt"
    ...
    user = ".\\라이어서버전용폴더\\" + rcode + "\\" + str(us) +  ".txt"      
...
def userjoblistappend(rcode):
    room = ".\\라이어서버전용폴더\\" + rcode + '\\' + rcode + ".txt"   
    user_list = ".\\라이어서버전용폴더\\" + rcode + '\\' + rcode + "user.txt"             
    with open(room,encoding='UTF8') as f:
        list = []
        tl = sum(1 for line in f)
        tl = tl + 1
...
def leave_room(us, rcode):
    ...
    finduser = ".\\라이어서버전용폴더\\" + rcode + '\\' + rcode + ".txt"   
...
def mafia_replace_data(user_id):#마피아 직업번호 1
    ...
    room = ".\\라이어전용폴더\\" + user_id + '\\' + user_id + "info.txt"
...
def user_data_file_replace_room(rcode, user_id):
    user = ".\\라이어전용폴더\\" + user_id + '\\' + user_id + "info.txt"
